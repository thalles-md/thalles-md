#  Thalles. (삼)

Atualmente estudando Python, C e Bash.

Meu objetivo a curto prazo é ser bom o suficiente nas ferramentas que uso
ao ponto de conseguir criar (quase) qualquer coisa que eu precise de forma
autônoma: ou seja, sem o uso de IA generativa para projetos pessoais. E inclusive até agora não há nada nesse perfil que seja feito com tais ferramentas.

Meu objetivo a longo prazo é usar a tecnologia para ajudar, informar, e proteger as pessoas.

Sou um estudante de Ciências da Computação, e escolhi esse curso por ver nele o maior potencial possível para ajudar as pessoas. Antes, tinha dúvida entre vários cursos de humanas (que, apesar de eu não gostar dessa divisão, poderia se dizer que seriam áreas que eu teria mais afinidade) como Psicologia, Letras, e etc. Mas vi na computação a chance de exercer minha criatividade e ideias com muito mais **liberdade** e **potencial**. 

Tenho imensa admiração pelo movimento FOSS (Free and Open Source Software), e acredito que não há como separar a tecnologia da filosofia, ou de qualquer outra área humana. E então, temos total responsabilidade com o quê criamos/disponibilizamos e também com o quê *deixamos de criar/disponibilizar*. 

Meu sistema atual é o **Fedora Linux**.
Você pode ver minhas "dotfiles" [aqui](https://github.com/thalles-md/thalles-md/tree/main/dotfiles).



---

<img width="500" height="279" text-align="center" alt="Face to Face" src="perseverança.jpg" />
